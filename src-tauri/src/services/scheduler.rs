...
async fn check_and_perform_backup<R: tauri::Runtime>(
    ...
    check_and_perform_backup_internal(
        &state.db,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_check_and_perform_backup_skips_when_not_needed() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let store = MockStore { settings: Mutex::new(Some(settings.clone())) };
