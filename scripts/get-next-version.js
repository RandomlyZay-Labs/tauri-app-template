...
async function run() {
	try {
		...
		if (result) {
			...
			console.log('No release is required (e.g. no relevant commits or chore/docs commits).');
			if (process.env.GITHUB_OUTPUT) {
		...
		console.error('Failed to determine next release version programmatically:', error);
		process.exit(1);
	}
}
